	...
	class Advance_Search_Widget extends WP_Widget {
		...
		public function __construct() {
			$widget_ops = array(
				...
				'description' => esc_html__( 'This widget displays advance search form.', ERE_TEXT_DOMAIN ),
			);
			parent::__construct( 'advance_search_widget', esc_html__( 'RealHomes - Advance Search Widget', ERE_TEXT_DOMAIN ), $widget_ops );
		...
		function widget( $args, $instance ) {
			...
			$title = ! empty( $instance['title'] ) ? $instance['title'] : esc_html__( 'Find Your Home', ERE_TEXT_DOMAIN );
			...
			if ( 'ultra' === INSPIRY_DESIGN_VARIATION ) {
                ...
                elseif ( ere_is_search_page_configured() && ( 'modern' === INSPIRY_DESIGN_VARIATION ) ) :
					...
					echo '<p class="alert alert-error">' . esc_html__( 'Advance search page is not configured yet. Please read this theme documentation for the Advance Search page & form configuration details.', ERE_TEXT_DOMAIN ) . '</p>';
				endif;
			}
		...
		function form( $instance ) {
			$instance = wp_parse_args(
				(array)$instance, array(
					'title' => esc_html__( 'Find Your Home', ERE_TEXT_DOMAIN ),
				)
			);
			...
			if ( 'ultra' === INSPIRY_DESIGN_VARIATION ) {
				...
				?>
                <p>
                    <label for="<?php echo esc_attr( $this->get_field_id( 'elementor_search_form' ) ); ?>"><?php esc_html_e( 'Select Elementor Search Form', ERE_TEXT_DOMAIN ); ?></label>

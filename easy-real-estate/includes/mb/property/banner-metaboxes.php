...
function ere_banner_metabox_tab( $property_metabox_tabs ) {
	if ( is_array( $property_metabox_tabs ) ) {
		$property_metabox_tabs['banner'] = array(
			'label' => esc_html__( 'Top Banner', ERE_TEXT_DOMAIN ),
...
function ere_banner_metabox_fields( $property_metabox_fields ) {
	...
	$ere_banner_fields = array(
		array(
			'name'             => esc_html__( 'Top Banner Image', ERE_TEXT_DOMAIN ),
			'id'               => "REAL_HOMES_page_banner_image",
			'desc'             => esc_html__( 'You can upload banner image specific for this property to override default one. Recommended minimum image size is 2000px by 230px.', ERE_TEXT_DOMAIN ),
			'type'             => 'image_advanced',
			'max_file_uploads' => 1,
			'columns'          => 12,
			'tab'              => 'banner',
	...
	if ( 'modern' == INSPIRY_DESIGN_VARIATION ) {
		$ere_banner_fields[] =
			array(
				'name'      => esc_html__( 'Hide Search Form in Header ?', ERE_TEXT_DOMAIN ),

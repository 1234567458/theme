...
if ( ! function_exists( 'inspiry_search_form_beds_customizer' ) ) :
	...
	function inspiry_search_form_beds_customizer( WP_Customize_Manager $wp_customize ) {
		...
		$wp_customize->add_section( 'inspiry_search_form_beds_baths', array(
			'title' => esc_html__( 'Search Form Beds & Baths', RH_TEXT_DOMAIN ),
		...
		if ( 'ultra' !== INSPIRY_DESIGN_VARIATION ) {
			...
			$wp_customize->add_setting( 'inspiry_min_beds_label', array(
				...
				'default'           => esc_html__( 'Min Beds', RH_TEXT_DOMAIN ),
				'sanitize_callback' => 'sanitize_text_field',
			) );
			$wp_customize->add_control( 'inspiry_min_beds_label', array(
				'label'   => esc_html__( 'Label for Min Beds Field', RH_TEXT_DOMAIN ),
				'type'    => 'text',
			...
			$wp_customize->add_control( 'inspiry_min_beds_placeholder', array(
				'label'   => esc_html__( 'Placeholder for Min Beds Field', RH_TEXT_DOMAIN ),
				'type'    => 'text',
				'section' => 'inspiry_search_form_beds_baths',
			) );
			...
			$wp_customize->add_control( 'inspiry_min_beds', array(
				'label'       => esc_html__( 'Minimum Beds Values', RH_TEXT_DOMAIN ),
			...
			$wp_customize->add_setting( 'inspiry_min_baths_label', array(
				'type'              => 'option',
				'transport'         => 'postMessage',
				'default'           => esc_html__( 'Min Baths', RH_TEXT_DOMAIN ),
				'sanitize_callback' => 'sanitize_text_field',
			) );
			$wp_customize->add_control( 'inspiry_min_baths_label', array(
				'label'   => esc_html__( 'Label for Min Baths Field', RH_TEXT_DOMAIN ),
				'type'    => 'text',
				'section' => 'inspiry_search_form_beds_baths',
			) );
			...
			$wp_customize->add_control( 'inspiry_min_baths_placeholder', array(
				'label'   => esc_html__( 'Placeholder for Min Baths Field', RH_TEXT_DOMAIN ),
				'type'    => 'text',
				'section' => 'inspiry_search_form_beds_baths',
			) );
			...
			$wp_customize->add_control( 'inspiry_min_baths', array(
				'label'       => esc_html__( 'Minimum Baths Values', RH_TEXT_DOMAIN ),
				'description' => esc_html__( 'Only provide comma separated numbers.', RH_TEXT_DOMAIN ),
				'type'        => 'textarea',
				'section'     => 'inspiry_search_form_beds_baths',
			) );
		...
		$wp_customize->add_control( 'inspiry_beds_baths_search_behaviour', array(
			'label'       => esc_html__( 'Beds and Baths Search Behaviour', RH_TEXT_DOMAIN ),
			'description' => esc_html__( 'Do you want the search functionality to look for minimum beds, maximum beds or exact equals ?', RH_TEXT_DOMAIN ),
			'type'        => 'select',
			'section'     => 'inspiry_search_form_beds_baths',
			'choices'     => array(
				'min'   => esc_html__( 'Minimum', RH_TEXT_DOMAIN ),
				'max'   => esc_html__( 'Maximum', RH_TEXT_DOMAIN ),
				'equal' => esc_html__( 'Equal', RH_TEXT_DOMAIN ),
			),
		) );

...
if ( ! function_exists( 'realhomes_hide_empty_property_types' ) ) :
	/**
	 * Determines whether to hide empty property types based on customizer settings or not.
     *
	 * @since 4.1.0
     *
	 * @return bool
	 */
	function realhomes_hide_empty_property_types(): bool {

		if ( 'true' == get_option( 'realhomes_hide_empty_property_types', 'true' ) ) {
			return true;
		}
	...
	function realhomes_locations_js_data() {
		...
		if ( ! is_admin() && class_exists( 'ERE_Data' ) ) {
			...
			if ( is_page_template( array( 'templates/dashboard.php' ) ) ) {
				$any_text = esc_html__( 'None', RH_TEXT_DOMAIN );  // modify it to None for submit page
			}
...
if ( ! function_exists( 'realhomes_hierarchical_options' ) ) {
	...
	function realhomes_hierarchical_options( $taxonomy_name ) {
		...
		if ( $taxonomy_name == 'property-type' ) {
			$hierarchical_terms_array   = ERE_Data::get_hierarchical_property_types( realhomes_hide_empty_property_types() );
...
if ( ! function_exists( 'inspiry_get_location_titles' ) ) :
	...
	function inspiry_get_location_titles() {
		...
		$main_location = esc_html__( 'Main Location', RH_TEXT_DOMAIN );

		if ( inspiry_is_rvr_enabled() ) {
			$main_location = esc_html__( 'City', RH_TEXT_DOMAIN );
		}

		$location_select_titles = array(
			$main_location,
			esc_html__( 'Child Location', RH_TEXT_DOMAIN ),
			esc_html__( 'Grand Child Location', RH_TEXT_DOMAIN ),
			esc_html__( 'Great Grand Child Location', RH_TEXT_DOMAIN ),
		);
...
if ( ! function_exists( 'realhomes_radius_search' ) ) :
	/**
	 ...
	 * @return array
	 */
	function realhomes_radius_search( $search_args ) {

		if ( ! empty( $_GET['geolocation-address'] ) && ! empty( $_GET['lat'] ) && ! empty( $_GET['lng'] ) ) {
			global $wpdb;

			$center_lat   = sanitize_text_field( $_GET['lat'] );
			$center_lng   = sanitize_text_field( $_GET['lng'] );
			$radius_range = ! empty($_GET['geolocation-radius']) ? $_GET['geolocation-radius'] : get_option( 'realhomes_search_radius_range_initial', '20' );
			...
			// Adding to args for save search feature
			$search_args['geolocation-address'] = $_GET['geolocation-address'];
			$search_args['geolocation-radius']  = $_GET['geolocation-radius'];
		}

		return $search_args;
	}

	add_filter( 'real_homes_search_parameters', 'realhomes_radius_search' );
endif;
...
if ( ! function_exists( 'inspiry_area_search' ) ) :
	...
	function inspiry_area_search( $meta_query ) {
		...
		if ( isset( $_GET['min-area'] ) && ! empty( $_GET['min-area'] ) && isset( $_GET['max-area'] ) && ! empty( $_GET['max-area'] ) ) {
			...
			// If minimum and maximum area is same
			...
			} else if ( $min_area >= 0 && $max_area > $min_area ) {
				$meta_query[] = array(
					'key'     => 'REAL_HOMES_property_size',
...
if ( ! function_exists( 'realhomes_lot_size_search' ) ) :
	...
	function realhomes_lot_size_search( $meta_query ) {
		...
		if ( isset( $_GET['min-lot-size'] ) && ! empty( $_GET['min-lot-size'] ) && isset( $_GET['max-lot-size'] ) && ! empty( $_GET['max-lot-size'] ) ) {
			...
			// If minimum and maximum lot is same
			...
			} else if ( $min_lot_size >= 0 && $max_lot_size > $min_lot_size ) {
				$meta_query[] = array(
					'key'     => 'REAL_HOMES_property_lot_size',
...
if ( ! function_exists( 'inspiry_price_search' ) ) :
	...
	function inspiry_price_search( $meta_query ) {
		...
		// Dual price check
		$dual_price_status = realhomes_is_dual_price_enabled();
			...
			if ( $min_price === $max_price ) {
				if ( $dual_price_status ) {
					$meta_query[] = array(
						...
						array(
							'key'     => 'REAL_HOMES_property_price_for_sale',
							'value'   => $min_price,
							'type'    => 'NUMERIC',
							'compare' => '='
						)
		...
		} else if ( isset( $_GET['min-price'] ) && ( $_GET['min-price'] != inspiry_any_value() ) ) {
			$min_price = doubleval( $_GET['min-price'] );
			if ( $min_price > 0 ) {
				if ( $dual_price_status ) {
					$meta_query[] = array(
		...
		} else if ( isset( $_GET['max-price'] ) && ( $_GET['max-price'] != inspiry_any_value() ) ) {
			$max_price = doubleval( $_GET['max-price'] );
			if ( $max_price > 0 ) {
				if ( $dual_price_status ) {
					$meta_query[] = array(

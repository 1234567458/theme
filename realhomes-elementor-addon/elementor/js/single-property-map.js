...
class RHEASinglePropertyMapWidgetClass extends elementorModules.frontend.handlers.Base {
    ...
    loadSinglePropertyMap( event ) {
        ...
        // Optimized Pin logic
        const getOptimizedPin = () => {
            const isRetina = window.devicePixelRatio >= 1.5;
            const marker   = elementSettings.map_marker?.url || '';
            const marker2x = elementSettings.retina_map_marker?.url || '';

            if ( isRetina && marker2x ) {
                return marker2x;
            }
        ...
        if ( mapLat && mapLong ) {
            ...
            if ( 'google-maps' === mapType ) {
                ...
                if ( mapboxAPI !== null ) {
                    ...
                    if ( pinUrl ) {
                        marker_icon_img.src          = pinUrl;
                        marker_icon_img.style.width  = '50px';
                        marker_icon_img.style.height = 'auto';
                    } else {
                    ...
                    const propertyMarker = new mapboxgl.Marker( marker_icon ).setLngLat( mapCenter ).addTo( propertyMap );
                ...
                if ( pinUrl ) {
                    ...
                    osmMarker = L.marker( [mapLat, mapLong], markerOptions ).addTo( map );
                } else {
                    osmMarker = L.marker( [mapLat, mapLong] ).addTo( map );
                }
